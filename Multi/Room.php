<?php
		  ...
		  if($ppl_inroom == $max_ppl){
			  echo "<script>alert(\"인원이 모두 찬 방입니다.\");</script>";
			  echo("<script>location.href='RoomList.html';</script>");
		  }

		  if (!in_array('$user_id', $arr)){
			  if ($ppl_inroom = 1 and $arr[0] != $user_id){
				$sql = "UPDATE room SET person_2='$user_id' WHERE Num_Room = '$num_room'";
				$dbConnect->query($sql);
				$sql = "UPDATE room SET ppl_inroom= 2 WHERE Num_Room = '$num_room'";
				$dbConnect->query($sql);
				$arr[1] = $user_id;
			  } else if($ppl_inroom = 2 and $arr[0] != $user_id){
				$sql = "UPDATE room SET person_3='$user_id' WHERE Num_Room = '$num_room'";
				$dbConnect->query($sql);
				$sql = "UPDATE room SET ppl_inroom= 3 WHERE Num_Room = '$num_room'";
				$dbConnect->query($sql);
				$arr[2] = $user_id;
			  } else if($ppl_inroom = 3 and $arr[0] != $user_id){
				$sql = "UPDATE room SET person_4='$user_id' WHERE Num_Room = '$num_room'";
				$dbConnect->query($sql);
				$sql = "UPDATE room SET ppl_inroom= 4 WHERE Num_Room = '$num_room'";
				$dbConnect->query($sql);
				$arr[3] = $user_id;
			  }
		  }
